...
async function callAI(messages: { role: string; content: string }[], system = "", maxTokens = 800) {
  try {
...
function Avatar({ talking, thinking, color, size = 120 }: { talking: boolean; thinking: boolean; color: string; size?: number }) {
...
interface Msg { role: string; content: string; typing?: boolean }

function MsgBubble({ msg, color, onSpeak }: { msg: Msg; color: string; onSpeak: (t: string) => void }) {
...
type VideoResult = { title?: string; duration?: string; hook?: string; scenes?: { time: string; visual: string; voiceover?: string }[]; music?: string; platform?: string; cta?: string; tips?: string };

export default function App() {
  const [modules, setModules]      = useState<Module[]>(INIT_MODULES);
  const [removedMods, setRemoved]  = useState<Module[]>([]);
  ...
  const [videoRes, setVideoRes]    = useState<VideoResult | null>(null);
  ...
  useEffect(() => {
    const t = setInterval(() => { removeOldModules(); }, 24 * 60 * 60 * 1000);
    return () => clearInterval(t);
  }, []);
    ...
    setModules(prev => {
      ...
      prev.forEach(m => {
        if (!m.locked && m.lastUsed < cutoff) {
          ...
          setLearnLog(p => [{
            time: new Date().toLocaleTimeString(), type: "remove",
            icon: m.icon, text: `"${m.name}" o'chirildi — 1 yil ishlatilmagan`,
          }, ...p].slice(0, 60));
        } else {
          active.push(m);
        }
  ...
  const discoverNew = useCallback(async () => {
    ...
    if (result.ok && result.text) {
      try {
        ...
        const parsed = JSON.parse(clean) as { name?: string; icon?: string; cat?: string; desc?: string; earning?: string; tech?: string };
  ...
  const sendMessage = useCallback(async (txtArg?: string) => {
    ...
    const sys = systemPrompts[agentId] ||
      `Sen Ulug'bek AI — ${agent?.name || "yordamchi"}. O'zbek tilida qisqa, foydali javob ber.`;
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: "auto", position: "relative", zIndex: 5 }}>
        ...
        {tab === "chat" && (
          <div style={{ padding: "12px 13px" }}>
            ...
            {messages.length === 0 ? (
              ...
              messages.filter((_, i, a) => !a[i].typing || i === a.length - 1)
                .map((m, i) => <MsgBubble key={i} msg={m} color={color} onSpeak={speakText} />)
            )}
            <div ref={endRef} />
          </div>
        )}
        ...
        {tab === "video" && (
          <div style={{ padding: "13px" }}>
            ...
            <div style={{ display: "flex", gap: 7, marginBottom: 9 }}>
              <input value={videoP} onChange={e => setVideoP(e.target.value)}
            ...
            {videoRes && (
              <div style={{
                ...
                {videoRes.hook && (
                  <div style={{ background: `${color}15`, borderRadius: 8, padding: "7px 10px", marginBottom: 8, color, fontSize: 12 }}>
                    🎣 Hook: {videoRes.hook}
                  </div>
                )}
        ...
        {tab === "modules" && (
          <div style={{ padding: "13px" }}>
            <div style={{ color, fontWeight: 700, fontSize: 14, marginBottom: 12 }}>🧩 Barcha Modullar ({activeMods.length})</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
              {modules.map(m => (
                <div key={m.id} onClick={() => { if (!m.locked) { setAgentId(m.id); setTab("chat"); } }} style={{
                  padding: "10px 11px", borderRadius: 12, cursor: m.locked ? "default" : "pointer",
                  background: "rgba(255,255,255,0.04)",
                  border: `1px solid ${m.auto ? `${color}44` : "rgba(255,255,255,0.07)"}`,
                  ...
                  {m.auto && (
                    <div style={{
                      ...
                      borderRadius: 5, padding: "1px 5px", fontSize: 8, color,
                    }}>AI</div>
                  )}
                  <div style={{ fontSize: 22, marginBottom: 5 }}>{m.icon}</div>
                  <div style={{ color: "rgba(255,255,255,0.82)", fontSize: 11.5, fontWeight: 600, marginBottom: 2 }}>{m.name}</div>
                  <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 10, marginBottom: 5 }}>{m.desc}</div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ color: "rgba(255,255,255,0.2)", fontSize: 9 }}>{m.uses}x · {timeLeft(m.lastUsed)}</div>
            ...
            {removedMods.length > 0 && (
              <div>
                <div style={{ color: "rgba(255,255,255,0.25)", fontSize: 10, fontWeight: 700, marginBottom: 7, textTransform: "uppercase", letterSpacing: .8 }}>
                  O'chirilgan modullar ({removedMods.length})
                </div>
                {removedMods.map((m, i) => (
                  <div key={i} style={{
        ...
        {tab === "learn" && (
          <div style={{ padding: "13px" }}>
            ...
            <div style={{
              ...
              <div style={{ color, fontWeight: 600, fontSize: 12, marginBottom: 7 }}>Avtomatik qoidalar:</div>
